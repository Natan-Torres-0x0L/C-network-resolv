...
resolv_answers(struct resolv_class *resolv_class, size_t resolv_class_size, resolv_class_type_t resolv_class_type, uint8_t *response, size_t length) {
  ...
  for (;;) {
    ...
    if (!network_dns_parser_resource(parser, &resource))
      break;
    ...
    switch (resource.rtype) {
      case NETWORK_DNS_TYPE_A: {
        if (resolv_class_type == RESOLV_HOSTENTRY) {
          ...
          if (!(hostentry->addrs = (char **)realloc(hostentry->addrs, (hostentry->naddrs+1)*sizeof(char *))))
            break;

          network_dns_parser_a(parser, &a);

          hostentry->addrs[hostentry->naddrs++] = string_new(a.a);
        }
      ...
      case NETWORK_DNS_TYPE_AAAA: {
        if (resolv_class_type == RESOLV_HOSTENTRY) {
          ...
          hostentry->addrs[hostentry->naddrs++] = string_new(aaaa.aaaa);
        }
      ...
      case NETWORK_DNS_TYPE_CNAME: {
        if (resolv_class_type == RESOLV_HOSTENTRY) {
          ...
          if (!(hostentry->names = (char **)realloc(hostentry->names, (hostentry->nnames+1)*sizeof(char *))))
            break;

          network_dns_parser_cname(parser, &cname);

          hostentry->names[hostentry->nnames++] = string_new(cname.cname);
        }
     // @ CNAME class
     //
     ...
     // }
      ...
      case NETWORK_DNS_TYPE_MX: {
        if (resolv_class_type == RESOLV_MX) {
          ...
          if (!(*((void **)&resolv_mx->mx) = realloc(resolv_mx->mx, (resolv_mx->nmx+1)*sizeof(*resolv_mx->mx))))
            break;

          network_dns_parser_mx(parser, &mx);

          resolv_mx->mx[resolv_mx->nmx].preference = mx.preference; 
          resolv_mx->mx[resolv_mx->nmx++].exchange = string_new(mx.exchange);
        }
      ...
      case NETWORK_DNS_TYPE_NS: {
        if (resolv_class_type == RESOLV_NS) {
          ...
          if (!(resolv_ns->nsnames = (char **)realloc(resolv_ns->nsnames, (resolv_ns->nnsnames+1)*sizeof(char *))))
            break;

          network_dns_parser_ns(parser, &ns);

          resolv_ns->nsnames[resolv_ns->nnsnames++] = string_new(ns.nsname);
        }
      ...
      case NETWORK_DNS_TYPE_PTR: {
        if (resolv_class_type == RESOLV_HOSTENTRY) {
          ...
          hostentry->names[hostentry->nnames++] = string_new(ptr.ptr);
        }
      ...
      case NETWORK_DNS_TYPE_TXT: {
        if (resolv_class_type == RESOLV_TXT) {
          ...
          if (!(resolv_txt->txts = (char **)realloc(resolv_txt->txts, (resolv_txt->ntxts+1)*sizeof(char *))))
            break;

          network_dns_parser_txt(parser, &txt);

          resolv_txt->txts[resolv_txt->ntxts++] = string_new(txt.txt);
        }
...
resolv_rcode_toerror(uint8_t rcode) {
...
static struct resolv_class *
resolv_search_byclass(struct resolv_search *search, const char *name, resolv_class_type_t resolv_class_type, size_t resolv_class_size) {
  ...
  if ((rcode = resolv_network_dnshdr_rcode(response))) {
    resolv_seterror(resolv_rcode_toerror(rcode));
    return NULL;
  }
...
resolv_hostentry_search(struct resolv_search *search, const char *name) {
  ...
  if (!(hostentry = (struct resolv_hostentry *)resolv_search_byclass(search, name, RESOLV_HOSTENTRY, sizeof(struct resolv_hostentry))))
    return NULL;
...
resolv_hostentry_reserved(const struct resolv_hostentry *reserved) {
  ...
  if (!(hostentry->names = (char **)calloc(reserved->nnames, sizeof(char *))))
    goto _return;

  for (hostentry->nnames = 0; hostentry->nnames < reserved->nnames; hostentry->nnames++)
    hostentry->names[hostentry->nnames] = string_new(reserved->names[hostentry->nnames]);
  hostentry->name = *hostentry->names;

  if (!(hostentry->addrs = (char **)calloc(reserved->naddrs, sizeof(char *))))
    goto _return;

  hostentry->addrs[hostentry->naddrs++] = string_new(*reserved->addrs);
...
static struct resolv_hostentry *
resolv_hostentry_reserved_byname(const struct resolv_hostentry *reserved, const char *name) {
  char **names = NULL;
...
static struct resolv_hostentry *
resolv_hostentry_reserved_byaddr(const struct resolv_hostentry *reserved, const char *addr) {
  char **addrs = NULL;
...
resolv_reserved(struct resolv_search *search, const char *name) {
  switch (search->question.qtype) {
    case NETWORK_DNS_TYPE_A:
      return resolv_hostentry_reserved_byname(resolv_hostentry4_reserved, name);

    case NETWORK_DNS_TYPE_AAAA:
      return resolv_hostentry_reserved_byname(resolv_hostentry6_reserved, name);

    case NETWORK_DNS_TYPE_PTR:
      if (inet_strisaddr(name) == INETV4_ADDRESS)
        return resolv_hostentry_reserved_byaddr(resolv_hostentry4_reserved, name);

      return resolv_hostentry_reserved_byaddr(resolv_hostentry6_reserved, name);
  }
...
resolv_hostentry_free(struct resolv_hostentry *hostentry) {
  if (hostentry) {
    ...
    if (hostentry->addrs) {
      do {
        free(hostentry->addrs[--hostentry->naddrs]);
        hostentry->addrs[hostentry->naddrs] = NULL;
      } while (hostentry->naddrs);

      free(hostentry->addrs), hostentry->addrs = NULL;
    }
    if (hostentry->names) {
      do {
        free(hostentry->names[--hostentry->nnames]);
        hostentry->names[hostentry->nnames] = NULL;
      } while (hostentry->nnames);
...
resolv_mx_search(struct resolv_search *search, const char *name) {
  ...
  if (!(mx = (struct resolv_mx *)resolv_search_byclass(search, name, RESOLV_MX, sizeof(struct resolv_mx))))
    return NULL;
...
resolv_mx_free(struct resolv_mx *mx) {
  if (mx) {
    ...
    if (mx->mx) {
      do {
        free(mx->mx[--mx->nmx].exchange);
        mx->mx[mx->nmx].exchange = NULL;
      } while (mx->nmx);
...
resolv_ns_search(struct resolv_search *search, const char *name) {
  ...
  if (!(ns = (struct resolv_ns *)resolv_search_byclass(search, name, RESOLV_NS, sizeof(struct resolv_ns))))
    return NULL;
...
resolv_ns_free(struct resolv_ns *ns) {
  if (ns) {
    if (ns->nsnames) {
      do {
        free(ns->nsnames[--ns->nnsnames]);
        ns->nsnames[ns->nnsnames] = NULL;
      } while (ns->nnsnames);
...
resolv_soa_search(struct resolv_search *search, const char *name) {
  ...
  if (!(soa = (struct resolv_soa *)resolv_search_byclass(search, name, RESOLV_SOA, sizeof(struct resolv_soa))))
    return NULL;
...
resolv_txt_search(struct resolv_search *search, const char *name) {
  ...
  if (!(txt = (struct resolv_txt *)resolv_search_byclass(search, name, RESOLV_TXT, sizeof(struct resolv_txt))))
    return NULL;
...
resolv_txt_free(struct resolv_txt *txt) {
  if (txt) {
    if (txt->txts) {
      do {
        free(txt->txts[--txt->ntxts]);
        txt->txts[txt->ntxts] = NULL;
      } while (txt->ntxts);
